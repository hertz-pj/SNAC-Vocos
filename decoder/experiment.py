...
class SnacVocos(VocosExp):
    def __init__(
        ...
        if self.feature_extractor.quantize_dropout and self.feature_extractor.rand_num_quant is not None:
            self.multiperioddisc = MultiPeriodDiscriminator(num_embeddings=len(self.feature_extractor.rand_num_quant))
            self.multiresddisc = MultiResolutionDiscriminator(num_embeddings=len(self.feature_extractor.rand_num_quant))
        else:
            self.multiperioddisc = MultiPeriodDiscriminator()
            self.multiresddisc = MultiResolutionDiscriminator()
        self.dac = DACDiscriminator()
    ...
    def training_step(self, *args):
        if self.feature_extractor.quantize_dropout and self.feature_extractor.rand_num_quant is not None:
            rand_num_id = torch.randint(
                low=0,
                high=len(self.feature_extractor.rand_num_quant),
                size=(1,),
                device=self.device,
            )
        else:
            rand_num_id = None
        output = super().training_step(*args, bandwidth_id=rand_num_id)
        return output

    def validation_step(self, *args):
        rand_num_id = (
            torch.tensor([len(self.feature_extractor.rand_num_quant) - 1], device=self.device)
            if self.feature_extractor.quantize_dropout and self.feature_extractor.rand_num_quant is not None
            else None
        )
        output = super().validation_step(*args, bandwidth_id=rand_num_id)
        return output
